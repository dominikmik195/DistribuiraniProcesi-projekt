...
public class Process implements MsgHandler {
    ...
    public void sendMsg(int destId, String tag, int msg1, ArrayList<Integer> length, ArrayList<Integer> parent) {
        sendMsg(destId,tag,String.valueOf(msg1)
        +" "+String.valueOf(length)+" "+String.valueOf(parent));
    }
    ...
    public boolean isNeighbor(int i) {
        if (comm.neighbours.contains(i)) return true;
        else return false;
    }
